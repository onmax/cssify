...
import type { Atrule, Block, CssNode, List, MediaQuery, MediaQueryList, PseudoClassSelector, Rule, SelectorList, StyleSheet } from 'css-tree'
import type { CSSRule } from '.'
...
function walkChildren(children: List<CssNode>) {
  ...
  for (const child of children) {
    ...
    if (child.type === 'Dimension')
      ...
      v += `${child.property}: ${child.value.type === 'Value' ? walkChildren(child.value.children) : _jsonify(child.value.value)}${child.important ? '!important' : ''}`
...
function _jsonify(content: string, rules: CSSRule = {}): CSSRule {
 ...
 * @returns the JSON representation of the CSS
 */
export function jsonify(content: string): CSSRule {
  return _jsonify(content)
}
